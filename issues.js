    ...
    $(".js-issues-list").find("a.js-navigation-open").each(function () {
      ...
      chrome.storage.sync.get(href, function (data) {
        ...
        if (voters.indexOf(my_username) !== -1) {
          $a.parents('.info-wrapper').append('<p style="position: absolute; right: -25px; top: 10px; "><img class="emoji" title=":+1:" alt=":+1:" src="https://a248.e.akamai.net/assets.github.com/images/icons/emoji/+1.png" height="20" width="20" align="absmiddle"></p>');
        }
        ...
        if (other_voters.length) {
          ...
          // Add number of votes
          $a.parents('.info-wrapper').append('<p style="position: absolute; right: -57px; top: 10px; font-size: 18px">+ ' + other_voters.length + '</p>');
        }
      });
    });
  }
